...
RUN env GOOS=$TARGETOS GOARCH=$TARGETARCH go build -modcacherw -o cloudsave-validator-grpc-plugin-server-go_$TARGETOS-$TARGETARCH
...
COPY --from=builder /build/cloudsave-validator-grpc-plugin-server-go_$TARGETOS-$TARGETARCH cloudsave-validator-grpc-plugin-server-go
...
# Entrypoint.
CMD [ "/app/cloudsave-validator-grpc-plugin-server-go" ]